	...
	return (
		<li className="bg-gray-50 bg-opacity-75 dark:bg-gray-900 dark:bg-opacity-75 backdrop-filter backdrop-blur-sm border border-gray-100 dark:border-gray-500 rounded-lg transition ease-in-out duration-300">
			<div className="flex flex-col sm:flex-row items-center justify-between px-4 py-4 sm:px-6">
				<div className="flex md:flex-row flex-col flex-1 items-center justify-start w-full">
					{icon &&
						(typeof icon === 'string' ? (
							<div
								className={clsx(
					...
					<div className="min-w-0 flex-1 px-4">
						...
						<div className="flex mt-2 w-full flex-wrap gap-2">
							{language?.map((item) => (
								<div className="w-fit px-4 py-[2px] rounded-lg text-sm text-black font-semibold bg-yellow-200">
									{item}
								</div>
							))}
						</div>
